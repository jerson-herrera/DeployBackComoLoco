...
export const obtenerUsersGanadores = async (req, res) => {
    try {
        // Encuentra todos los intentos donde el código está asociado a un premio
        const intentos = await IntentosModel.find()
            .populate({
                path: 'userId',
                select: 'Nombre Cedula Celular', // Selecciona solo los campos que necesitamos del UserInfo
            })
            .populate({
                path: 'codigo', // Asegúrate de que el campo se llame `codigo` en el esquema de intentos
                model: CodigoModel,
                match: { TienePremio: true }, // Solo selecciona códigos que tienen premio
                select: 'Codigo Premio', // Selecciona solo los campos necesarios de Codigo
            });

        // Filtra los intentos para que solo queden los que tienen premio
        const ganadores = intentos.filter(intento => intento.codigo);

        // Mapea la información que queremos devolver
        const resultados = ganadores.map(intento => ({
            FechaUso: intento.fecha,
            Nombre: intento.userId.Nombre,
            Cedula: intento.userId.Cedula,
            Celular: intento.userId.Celular,
            CodigoGanador: intento.codigo.Codigo,
            Premio: intento.codigo.Premio,
        }));

        return res.status(200).json(resultados);
    } catch (error) {
        console.error('Error al obtener ganadores:', error);
        return res.status(500).json({ message: 'Error al obtener ganadores' });
    }
};